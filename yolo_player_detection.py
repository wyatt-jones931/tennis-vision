...
@author: wyatt
"""
def process_video(input_name, save_csv=False):    
    import os
    import cv2
    import numpy as np
    ...
    # the model for player position detection
    ...
    # Extra space on the outside of the court expected
    RUNOFF_X = 12
    RUNOFF_Y = 21
    ...
    # Convets the image/pixel space to real space using homography matrix
    ...
    for filename in frame_files:
        ...
        if box is not None:
            ...
            # Only track player movement when they are on the side of the court with the camera
            if real_y <= NET_Y:
                ...
                # Stuff for CSV
                speed_mph = smoothed_speed * 0.681818 # Calms the estimated speed since it's not the full video
        ...
        # Only draw to minimap if we have a valid position
